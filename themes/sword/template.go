...
{{end}}`, "components/form/number_range": `{{define "form_number_range"}}
    {{if .Editable}}
        <div class="input-group number-range">
            <input style="text-align: center;" type="text" id="{{.Field}}_start__admin"
                   name="{{.Field}}_start__admin"
                   value="{{.Value}}" class="form-control {{.Field}}_start__admin"
                   placeholder="{{.Head}}">
            <span class="input-group-addon" style="border-left: 0; border-right: 0;">-</span>
            <input style="text-align: center;" type="text" id="{{.Field}}_end__admin" name="{{.Field}}_end__admin"
                   value="{{.Value2}}" class="form-control {{.Field}}_end__admin"
                   placeholder="{{.Head}}">
        </div>
        <script>
            $(function () {
                $('.{{.Field}}_start__admin:not(.initialized)')
                    .addClass('initialized')
                    .bootstrapNumber({
                        upClass: 'success',
                        downClass: 'primary',
                ...
                $('.{{.Field}}_end__admin:not(.initialized)')
                    .addClass('initialized')
                    .bootstrapNumber({
                        upClass: 'success',
                        downClass: 'primary',
...
{{end}}`, "footer": `{{define "footer"}}
    <footer class="main-footer">
        ...
        <strong>Powered by <a href="https://github.com/backyio/go-admin/go-admin">GoAdmin</a>.</strong>
        {{.FooterInfo}}
    </footer>
